...
ls = [24, 23, 22, 21, 19, 18, 17]
for y in ls:
    data = {}
    wards = []

    with open(f'./geodata/cuas/cua-20{y}.geojson') as f:
        d = json.load(f)
        for i in d['features']:
            wards.append(i['properties'][f'CTYUA{y}CD'])
    ...
    for code in wards:        
        ...
        if code not in data.keys():
            print(y, code)
            data[code] = "NONE"
    with open(f'./data/20{y}/20{y}-cuas-past-elections.json', 'w') as h:
